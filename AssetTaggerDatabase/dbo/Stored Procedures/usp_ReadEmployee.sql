CREATE PROCEDURE [dbo].[usp_ReadEmployee]
    ...
    @FromEmployeeInsertDate DATETIME = NULL,
    @ToEmployeeInsertDate DATETIME = NULL,
    @RowsToSkip INT = NULL,
...
BEGIN
    ...
    DECLARE @ReadEmployee BIT = (SELECT ReadEmployee FROM [dbo].[tvf_GetCRUDPermissionsOfEndUser](@CallingEndUserID));

    IF (@ReadEmployee IS NULL)
        BEGIN
    ...
    ORDER BY
        CASE WHEN @NewestRowsFirst = 1 THEN EmployeeNumber END DESC,
