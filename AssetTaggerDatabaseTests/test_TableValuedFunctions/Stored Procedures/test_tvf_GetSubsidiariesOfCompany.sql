...
CREATE PROCEDURE [test_TableValuedFunctions].[test_tvf_GetSubsidiariesOfCompany]
AS
BEGIN
    ...
    -- Expected output.
    CREATE TABLE #expected (SubsidiaryID UNIQUEIDENTIFIER);
    ...
    -- -- Actual output.
    CREATE TABLE #actual (SubsidiaryID UNIQUEIDENTIFIER);

    INSERT INTO #actual (SubsidiaryID)
    SELECT SubsidiaryID FROM [dbo].[tvf_GetSubsidiariesOfCompany](@CompanyID01);
